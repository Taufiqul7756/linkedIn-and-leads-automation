...
| `id` | Asked when |
|---|---|
| `tone` | The message said nothing about how the posts should sound. Options: `professional`, `conversational`, `bold`, `storytelling`, `inspirational`, `educational`, `analytical`. A tone stated in the prompt ("write three bold, contrarian posts") is read off the message and this is **not** asked |
...
* **`count`, `emoji`, `hashtags`, `knowledge` and `AI image` are never asked
  ...
  default.
* **`tone` and `length` are asked only when the message did not state them.**
  Both are read off the prompt now, so a message that names a tone and a length
  produces a round with neither form question in it — and a round with nothing
  else to ask is skipped entirely.
...
* `body_blocks` — the same post as a **Tiptap (ProseMirror) document**. Load it
  into the editor; fall back to `body` if it is `{}`.
...
"body_blocks": {
  ...
  "content": [
    {"type": "paragraph", "content": [
      {"type": "text", "text": "Most GenAI demos hide a "},
      {"type": "text", "text": "dirty secret", "marks": [{"type": "bold"}]},
      {"type": "text", "text": ". 🤫"}
    ]},
    {"type": "paragraph", "content": [
      {"type": "text", "text": "What actually moved the needle:"}
    ]},
    {"type": "bulletList", "attrs": {"marker": "-", "tight": true}, "content": [
      {"type": "listItem", "content": [
        {"type": "paragraph", "content": [
          {"type": "text", "text": "Celery + Redis → "},
          {"type": "text", "text": "80%", "marks": [{"type": "bold"}]},
          {"type": "text", "text": " faster"}
        ]}
      ]},
      {"type": "listItem", "content": [
        {"type": "paragraph", "content": [
          {"type": "text", "text": "pgvector, so no second database bill"}
        ]}
      ]}
    ]}
  ]
...
Generation only ever writes `paragraph`, `bulletList` and `bold` — the three
markers the model is allowed (`**bold**`, `- `/`→ ` lines, blank lines). The
editor may send back anything in its own schema; it is stored as sent.

Two attributes are ours and Tiptap's default schema does not declare them:

| Attribute | Meaning |
|---|---|
| `bulletList.attrs.marker` | The literal glyph the writer used: `-`, `*`, `•` or `→`. It is what publishes |
| `attrs.tight` | `true` = no blank line between this block and the one above |

Declare them on your `bulletList` / `paragraph` extensions if you want them
preserved. Drop them and nothing breaks: the next save re-derives the body with
`-` bullets and a blank line between every block.

**Editing.** `PATCH content/posts/{id}/` with `body_blocks` set to the editor's
document. It is stored byte for byte and **`body` is re-derived from it** —
so the document is the author and a `body` sent in the same request is ignored.
`PATCH`ing `body` alone still works (markdown-lite accepted — `**bold**`,
`- `/`→ ` lines) and writes the document instead. Anything that is not a
document (`{"type": "doc", …}`) is a `400` on `body_blocks`; the pre-Tiptap
block array is refused.

**What LinkedIn shows.** One plain string, so only characters survive: bullets,
arrows, emoji and blank lines publish as-is, **bold** becomes Unicode math-bold
letters, and everything else flattens — a heading is its own line, a
`blockquote` is its text, an `orderedList` numbers from `attrs.start`, a
`horizontalRule` publishes as `———`, and italic / strike / code publish as
plain characters. Say so in the editor rather than promising formatting the
network has no form for.
...
**With `use_hashtags` on** (the default), the tags are in **two** places and
they agree: the `hashtags` array, and the last line of `body` / `body_blocks`
(`#saas #pricing`, one paragraph, after a blank line). Render the body as it
stands — do not append the array under it, or every post shows its tags twice.
A rewrite and a Make-Longer both recompose that line, so it is never doubled and
an extension is added *above* it.

Editing is yours: `PATCH body`/`body_blocks` is stored as sent, tag line
included. If you change the `hashtags` array without changing the body, the
publisher appends whatever the body is missing — so nothing is lost, but the two
will read differently in the editor until the body is updated too.

**With `use_hashtags` off**, `hashtags` is `[]`, there is no tag line in the
body, and both stay that way through a rewrite.

A hashtag asked for **in the prompt** ("add some hashtags") turns them on for
that turn even when the panel's switch is off — the prompt outranks the panel,
in both directions, like every other setting.
...
| `use_hashtags` | `true` | On → 2–4 tags, in the `hashtags` array **and** on the last line of the body. Off → none anywhere, and none on a later rewrite |
...
GET    content/posts/?state=agent                     200  → the drafts, with body_blocks (Tiptap doc)
