...
const products = [
  {
    ...
    icon: <FaLinkedinIn className="h-4 w-4 text-teal-400" />,
  },
  {
    name: "Leads",
    href: "/leads",
    desc: "Find and manage your ideal prospects",
    icon: <LuUsers className="h-4 w-4 text-blue-400" />,
  },
];
...
export default function LandingNavbar() {
  ...
  return (
    <nav className="fixed left-0 right-0 top-0 z-50 border-b border-white/10 bg-slate-900/95 backdrop-blur-sm">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        ...
        <Link href="/" className="flex items-center gap-2.5">
          ...
          <span className="text-lg font-bold text-white">Relay</span>
        </Link>
        ...
        <div className="hidden items-center gap-8 md:flex">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {productsOpen && (
              <div className="absolute left-0 top-full mt-2 w-64 rounded-xl border border-slate-700 bg-slate-800 p-2 shadow-xl">
                {products.map((p) => (
                  <Link
                    key={p.name}
                    href={p.href}
                    onClick={() => setProductsOpen(false)}
                    className="flex items-start gap-3 rounded-lg px-4 py-3 transition-colors hover:bg-slate-700"
                  >
                    <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-slate-700">
                      {p.icon}
                    </div>
                    <div>
          ...
          <span className="flex cursor-default items-center gap-1.5 text-sm text-slate-500">
            Pricing
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="border-t border-slate-700 bg-slate-900 px-6 py-5 md:hidden">
          ...
          <div className="mt-5 flex items-center gap-4 border-t border-slate-700 pt-5">
            <Link href="/login" className="text-sm text-slate-300 hover:text-white">
              Login
            </Link>
            <Link
              href="/register"
              className="rounded-lg bg-teal-400 px-4 py-2 text-sm font-semibold text-slate-900"
            >
              Free Trial
            </Link>
          </div>
